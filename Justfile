...
bench *args:
    ...
    # Quick list for fast testing (curated for fewer deps)
    QUICK_PACKAGES=(
        "jq" "tree" "htop" "bat" "fd" "ripgrep" "fzf" "wget" "curl" "git" "tmux" "zoxide"
        "openssl@3" "sqlite" "readline" "pcre2" "zstd" "lz4" "node" "go" "ruby" "gh"
    )
    ...
    if [[ "$NO_COLOR" == "true" ]]; then
        ...
        C_RED="\x1b[31m" C_GREEN="\x1b[32m" C_YELLOW="\x1b[33m"
        C_BLUE="\x1b[34m" C_CYAN="\x1b[36m" C_NC="\x1b[0m"
    fi

    if [[ -n "$LOG_FILE" ]]; then
        : > "$LOG_FILE"
        echo -e "${C_BLUE}Debug logging to: $LOG_FILE${C_NC}" >&2
    fi

    if [[ "$DRY_RUN" == "true" ]]; then
        echo -e "${C_CYAN}=== Dry Run ===${C_NC}" >&2
    ...
    for i in "${!PACKAGES[@]}"; do
        pkg="${PACKAGES[$i]}"
        idx=$((i + 1))
        echo -e "${C_CYAN}[$idx/${#PACKAGES[@]}] Testing: $pkg${C_NC}" >&2
        ...
        if BREW_MS=$(run_timed_install "Homebrew" brew install "$pkg"); then
            echo -e "    ${C_GREEN}OK: ${BREW_MS}ms${C_NC}" >&2
        else
            echo -e "    ${C_RED}FAILED${C_NC}" >&2
            FAILED_NAMES+=("$pkg")
        ...
        if ZB_COLD_MS=$(run_timed_install "Zerobrew (cold)" zb install "$pkg"); then
            echo -e "    ${C_GREEN}OK: ${ZB_COLD_MS}ms${C_NC}" >&2
        else
            echo -e "    ${C_RED}FAILED${C_NC}" >&2
            FAILED_NAMES+=("$pkg")
        ...
        if ZB_WARM_MS=$(run_timed_install "Zerobrew (warm)" zb install "$pkg"); then
            echo -e "    ${C_GREEN}OK: ${ZB_WARM_MS}ms${C_NC}" >&2
        else
            echo -e "    ${C_RED}FAILED${C_NC}" >&2
            FAILED_NAMES+=("$pkg")
    ...
    if [[ -n "$FULL_OUTPUT_DIR" ]]; then
        ...
        echo -e "${C_CYAN}Writing all formats to: $FULL_OUTPUT_DIR${C_NC}" >&2

        FORMAT="text" output_result > "$FULL_OUTPUT_DIR/${BASE_NAME}.txt"
        echo -e "${C_GREEN}  ✓ ${BASE_NAME}.txt${C_NC}" >&2

        FORMAT="json" output_result > "$FULL_OUTPUT_DIR/${BASE_NAME}.json"
        echo -e "${C_GREEN}  ✓ ${BASE_NAME}.json${C_NC}" >&2

        FORMAT="csv" output_result > "$FULL_OUTPUT_DIR/${BASE_NAME}.csv"
        echo -e "${C_GREEN}  ✓ ${BASE_NAME}.csv${C_NC}" >&2

        FORMAT="html" output_result > "$FULL_OUTPUT_DIR/${BASE_NAME}.html"
        echo -e "${C_GREEN}  ✓ ${BASE_NAME}.html${C_NC}" >&2

        echo -e "${C_GREEN}All results written to: $FULL_OUTPUT_DIR${C_NC}" >&2
    elif [[ -n "$OUTPUT" ]]; then
        output_result > "$OUTPUT"
        echo -e "${C_GREEN}Results written to: $OUTPUT${C_NC}" >&2
    else
        output_result
    fi
